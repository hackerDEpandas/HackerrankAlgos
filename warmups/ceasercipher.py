...
if K > 26: # case where the key value is larger than 26
    K = K % 26 # change K to K mod 26 (ex: if K = 56, then 56 = 4 mod 26, and K would equal 4)

finalStr = '' # initialize empty string

for symbol in S: # begin iterating through each symbol in the string
    translation = '' # a place to store the symbol for each iteration

    if symbol.isalpha(): # case where symbol is in the alphabet
        ...
        if symbol.isupper(): # case where symbol is an uppercase letter
            ...
            if num > ord('Z'): # case where num is bigger than [90 = ord('Z')]
                num -= 26 # assign num to the proper ordinal number

        elif symbol.islower(): # case where symbol is a lowercase letter
            
            if num > ord('z'): # case where num is bigger than [122 = ord('z')]
                num -= 26 # assign num to the proper ordinal number
        
        translation += chr(num) # updates translation with the new encoded letter        
...
print finalStr # print the finished encoded message
